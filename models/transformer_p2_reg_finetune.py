...
class get_model(nn.Module):
    ...
    def forward(self, fix: torch.Tensor, mov: torch.Tensor,epoch=None,total_epoch=None):
        ...
        if total_epoch is not None and epoch is not None:
            rate=0.1*((total_epoch-epoch)-1/total_epoch)
        return warped1,loss1,reg_loss*rate
